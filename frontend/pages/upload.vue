...
const form = ref({
  ...
  evaluatee_id: '',        // เฉพาะ evaluator; evaluatee จะถูกอิงจาก JWT ฝั่ง server
  note: '',
  file: null
})
...
async function init() {
  ...
  try {
    ...
    periods.value = (pds || []).map(p => ({ ...p, name_th: p.name_th ?? p.name ?? p.title }))
    if (periods.value.length) {
      if (!form.value.period_id) form.value.period_id = periods.value[0].id
      await loadIndicators()
    } else {
      errorMsg.value = 'ยังไม่มีรอบการประเมินที่เปิดใช้งาน'
    }
  } catch (e) {
...
async function loadIndicators() {
  ...
  try {
    ...
    indicators.value = (data || []).map(i => ({ ...i, name_th: i.name_th ?? i.title ?? i.name }))
  } catch (e) {
    errorMsg.value = e.response?.data?.message || e.message
  }
}
...
async function loadEvidenceTypes() {
  ...
  try {
    ...
    evidenceTypes.value = (data || []).map(x => {
      const list =
        Array.isArray(x.mime_list) ? x.mime_list :
        (typeof x.mime_csv === 'string' ? x.mime_csv.split(',').map(s => s.trim()).filter(Boolean) : [])
      return { ...x, mime_list: list, max_mb: Number(x.max_mb ?? 10), name_th: x.name_th ?? x.name }
    })
  } catch (e) {
    errorMsg.value = e.response?.data?.message || e.message
  }
...
async function submit() {
  ...
  try {
    await $api.post('/api/attachments', fd, {
      ...
      onUploadProgress: (ev) => { if (ev.total) progress.value = Math.round((ev.loaded * 100) / ev.total) }
    })
...
watch(() => form.value.file, () => { if (errorMsg.value) errorMsg.value = '' })
  ...
  <div class="container mx-auto px-4 py-6">
    ...
    <v-card>
      <v-card-text>
        ...
        <v-skeleton-loader v-if="loadingInit" type="article" class="mb-4"/>
        ...
        <div v-else class="grid md:grid-cols-2 gap-6">
          <div>
            <v-select
              ...
              :rules="[rules.select]"
              density="comfortable" class="mb-3" clearable
            />
            <v-select
              v-model="form.indicator_id"
              :items="indicators"
              :item-title="(i)=> i.code ? `${i.code} — ${i.name_th}` : i.name_th"
              item-value="id"
              label="ตัวชี้วัด (Indicator)"
              :rules="[rules.select]"
              density="comfortable" class="mb-3" clearable
              :disabled="!form.period_id || !periods.length"
            />
            <v-select
            ...
            />
            <v-text-field
              v-if="auth.role === 'evaluator'"
              v-model="form.evaluatee_id"
              label="Evaluatee ID (กรณีอัปโหลดแทน)"
              density="comfortable" class="mb-3"
              hint="ต้องเป็นคู่ที่ถูกมอบหมายในงวดนี้" persistent-hint
            />
            <v-textarea v-model="form.note" label="หมายเหตุ/คำอธิบายไฟล์ (ไม่บังคับ)"
              auto-grow rows="2" density="comfortable" class="mb-3"/>
          </div>

          <div>
            <v-file-input
              v-model="form.file"
              :accept="allowMimes.length ? allowMimes.join(',') : '*/*'"
              label="เลือกไฟล์หลักฐาน" prepend-icon="mdi-paperclip"
            ...
            <v-alert variant="tonal" type="info" class="mt-3">
              <div class="text-sm">
                <div><strong>ชนิดไฟล์ที่อนุญาต:</strong> {{ allowMimes.length ? allowMimes.join(', ') : 'ยังไม่ระบุ (โปรดเลือกชนิดหลักฐาน)' }}</div>
                <div><strong>ขนาดสูงสุด:</strong> {{ maxMb || 0 }} MB</div>
        ...
        <div class="flex items-center gap-3">
          <v-btn color="primary"
            :disabled="submitting || !periods.length || !form.period_id || !form.indicator_id || !form.evidence_type_id || !form.file"
            @click="submit">Upload</v-btn>
          <v-progress-linear v-if="submitting || progress>0" :model-value="progress" height="6" striped class="flex-1"/>
        </div>
      </v-card-text>
    </v-card>
